...
describe("AgentClient", () => {
  ...
  it("prefers https and falls back to localhost http port when enabled", async () => {
    const calls: string[] = [];
    let attempts = 0;
    const client = new AgentClient({
  ...
  it("verifies expected certificate fingerprint before authenticated requests", async () => {
    const calls: string[] = [];
    const client = new AgentClient({
      baseUrl: "https://127.0.0.1:41527",
      getToken: () => "token_123",
